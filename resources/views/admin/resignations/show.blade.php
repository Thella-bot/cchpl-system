        ...
        <div class="col-md-5">
            @if($resignation->status === 'pending')
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.resignations.acknowledge', $resignation) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input
                                    type="number"
                                    id="balance_outstanding"
                                    class="form-control"
                                    step="0.01"
                                    min="0"
                                    value="{{ $resignation->balance_outstanding ?? 0 }}"
                                    readonly
                                >
                                <div class="form-text">Computed automatically.</div>
                            </div>
                ...
                <div class="card shadow-sm border-danger">
                    ...
                    <div class="card-body">
                        <p class="small text-muted">Rejecting will mark this resignation as rejected and record the event in the audit log.</p>

                        <form action="{{ route('admin.resignations.reject', $resignation) }}" method="POST">
                            @csrf

                            <div class="mb-3">
                                <label for="rejection_notes" class="form-label">Rejection Notes (Optional)</label>
                                <textarea name="rejection_notes" id="rejection_notes" class="form-control" rows="3" placeholder="Notes for internal record..."></textarea>
                            </div>

                            <div class="form-check mb-3">
                                <input class="form-check-input" type="checkbox" name="confirm_rejection" id="confirm_rejection" required>
                                <label class="form-check-label" for="confirm_rejection">
                                    I confirm that this resignation should be rejected.
                                </label>
                            </div>
            ...
            @else
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        @if(!empty($resignation->acknowledgement_notes))
                            <p><strong>Notes:</strong> {{ $resignation->acknowledgement_notes }}</p>
                        @else
